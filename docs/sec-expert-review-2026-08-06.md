...
- **Dependency vulnerabilities — actually scanned this time** (Round 1 incorrectly bucketed `npm audit` as "out of reach"; it is not). `packages/server` went from 27 to 13 findings (4 CRITICAL closed — all traced to `@simplewebauthn/server@5.4.3`'s `elliptic`/`jsrsasign` chain). **Scope correction**: this CRITICAL chain existed only in `packages/server` (the alternative self-hosted Node backend); the actually-deployed Cloudflare Worker API (`packages/worker`) has its own independent WebAuthn implementation that was already on `@simplewebauthn/server` v13+ before this round and was never on the vulnerable v5.4.3 — the live Cloudflare-hosted system's MFA signature verification was never exposed to this CVE chain. This fix matters for anyone running the self-hosted/Docker deployment path, not for the primary Cloudflare deployment. `packages/server`'s copy is now bumped to v13.3.2 too, with the integration re-verified against a real registration+authentication round-trip test.
...
## Post-Archive Correction — Round 3 (small follow-up, same session)

After Round 2 was archived, an external review of the handoff flagged two accuracy gaps before the branch was merged. Both were independently verified against real code (not taken on faith) and fixed:

1. **Report overstated the deployed impact of the WebAuthn CRITICAL findings.** The "Zero unaddressed CRITICAL/HIGH" framing and the "MFA signature-verification path" language did not make clear that the `@simplewebauthn/server@5.4.3` CRITICAL chain was scoped to `packages/server` (the self-hosted alternative backend) only — the actually-deployed Cloudflare Worker API had its own independent, already-safe WebAuthn implementation the whole time. Corrected in the Round 2 section above.
2. **A real, previously-undisclosed residual gap in the password-change session-revocation fix (R2-H3).** That fix cleared `auth.sessions` (completed sessions) but not `auth.srpSessions` — pending, not-yet-completed SRP handshakes initialized against the OLD verifier at `startCreateSession`/`startAuthRequest` time. Without clearing these, someone who already started a handshake using a compromised old password could still complete authentication with it for up to `auth.srpSessions`' 1-hour expiry window, even after the account owner changed their password. **Fixed**: `packages/core/src/server.ts`'s `updateAuth()` now also clears `auth.srpSessions` when the verifier changes, closing the same class of residual-access problem for in-flight handshakes that R2-H3 already closed for completed sessions.
   - **Verified**: added a real e2e test (`packages/worker/test/auth-flow-e2e.worker.ts`, "Password change invalidates a pending SRP handshake started with the old password") that starts a genuine SRP handshake with the old password, changes the password via a separate session, then attempts to complete the pending handshake — confirmed it now fails with `INVALID_SESSION`. Confirmed the test genuinely catches the pre-fix bug via a `git stash` baseline comparison (failed before the fix, passes after). Full `packages/worker` `test:ci` re-run: 0 failures.
   - **Severity**: bounded/narrow — requires an attacker to already possess the compromised old password AND have a still-open pending handshake at the moment of the password change; not independently exploitable without that precondition. Still a real completeness gap in the "revoke access on credential change" guarantee, and now closed.

With both corrections applied, the "zero unaddressed CRITICAL/HIGH" claim now holds accurately, and the WebAuthn dependency fix's real-world impact (self-hosted deployments only) is stated precisely rather than implied to cover the live Cloudflare deployment.
